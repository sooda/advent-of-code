...
fn neighbourhood(map: &Map, x: i32, y: i32, z: i32) -> usize {
    let mut total = 0;
    for dx in -1..=1 {
        for dy in -1..=1 {
            for dz in -1..=1 {
                if dx != 0 || dy != 0 || dz != 0 {
                    total += map.contains(&(x + dx, y + dy, z + dz)) as usize;
                }
            }
        }
    }
...
fn update_cell(map: &Map, x: i32, y: i32, z: i32) -> bool {
    let n = neighbourhood(map, x, y, z);
...
fn dimensions(map: &Map) -> ((i32, i32), (i32, i32), (i32, i32)) {
    ...
    for x in (dim.0.0 - 1)..=(dim.0.1 + 1) {
        for y in (dim.1.0 - 1)..=(dim.1.1 + 1) {
            for z in (dim.2.0 - 1)..=(dim.2.1 + 1) {
                let state = update_cell(map, x, y, z);
                if state {
                    next.insert((x, y, z));
                } else {
                    next.remove(&(x, y, z));
...
fn animate(boot_state: &Vec<Vec<char>>, n: usize) -> usize {
